...
struct StationCell: View {
    ...
    var body: some View {
        HStack {
            ZStack {
                VStack {
                    Rectangle().frame(width: 10.0).foregroundColor(type == .top
                                                                    ? .clear
                                                                    : Color(rgbValue: 0x00ced1))
                    Rectangle().frame(width: 10.0).foregroundColor(type == .bottom
                                                                    ? .clear
                                                                    : Color(rgbValue: 0x00ced1))
                }

                Circle().frame(width: 20.0).foregroundColor(Color(rgbValue: 0xffffff))
